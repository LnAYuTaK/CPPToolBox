...
bool Cabinet<T>::update(const Token &token, T *obj) {
  if (token.isNull() || token.pos() >= cells_.size()) return false;
...
T *Cabinet<T>::free(const Token &token) {
  if (token.isNull() || token.pos() >= cells_.size()) return nullptr;
...
T *Cabinet<T>::at(const Token &token) const {
  ...
  if (cell.id == token.id()) return cell.obj_ptr;
  return nullptr;
}
...
void Cabinet<T>::foreach (Func func) {
  for (auto &cell : cells_) {
    if (cell.id != 0) func(cell.obj_ptr);
  }
}
...
Id Cabinet<T>::allocId() {
  //! 避免分配 0 作为 id
  if (last_id_ == std::numeric_limits<Id>::max()) last_id_ = 0;
