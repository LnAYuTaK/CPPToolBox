...
bool Terminal::execute(SessionContext *s) {
  ...
  Utils::Split(s->curr_input, ";", cmdlines);
...
bool Terminal::executeCmd(SessionContext *s, const std::string &cmdline) {
  ...
  CLOG_INFO() << "cmdline: " << cmdline.c_str();
  vector<string> args;
  if (!Utils::SplitCmdline(cmdline, args) || args.empty()) {
      ...
      while (!node_token_stack.empty()) {
        ...
        while (!last_level.empty()) {
          ...
          if (curr_node->type() == NodeType::kFunc) {
            ...
            for (size_t i = 0; !is_repeat && i < node_token_stack.size() - 1;
                 ++i) {
