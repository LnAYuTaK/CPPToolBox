...
bool Terminal::execute(SessionContext *s)
{
    std::vector<std::string> cmdlines;
    Split(s->curr_input, ";", cmdlines);
...
bool Terminal::executeCmd(SessionContext *s, const std::string &cmdline)
{
    if (cmdline.empty()){
        return false;
    }
    std::vector<std::string> args;
    if (SplitCmdline(cmdline, args) || args.empty()) {
...
void Terminal::executeCdCmd(SessionContext *s, const Args &args)
{
    std::string path_str = "/";
    if (args.size() >= 2)
        path_str = args[1];
    ...
    if (is_found) {
        auto top_node_token = node_path.empty() ? rootNode_ : node_path.back().second;
...
void Terminal::executeHelpCmd(SessionContext *s, const Args &args)
{
    if (args.size() < 2) {
        printHelp(s);
        return;
...
void Terminal::executeLsCmd(SessionContext *s, const Args &args)
{
    std::string path_str = ".";
    if (args.size() >= 2)
        path_str = args[1];
...
void Terminal::executeHistoryCmd(SessionContext *s, const Args &args)
{
    stringstream ss;
...
void Terminal::executeExitCmd(SessionContext *s, const Args &args)
{
...
void Terminal::executeTreeCmd(SessionContext *s, const Args &args)
{
    string path_str = ".";
    if (args.size() >= 2)
        path_str = args[1];
...
void Terminal::executePwdCmd(SessionContext *s, const Args &args)
{
    std::stringstream ss;
    ss << '/';
...
bool Terminal::executeRunHistoryCmd(SessionContext *s, const Args &args)
{
    std::string sub_cmd = args[0].substr(1);
    if (sub_cmd == "!") {
...
void Terminal::executeUserCmd(SessionContext *s, const Args &args)
{
    std::stringstream ss;
