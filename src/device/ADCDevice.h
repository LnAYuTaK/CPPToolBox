...
class ADCDevice : public IODevice {
 ...
 public:
  ...
  ADCDevice();
  ~ADCDevice() override;
  bool init(const char *i2cPort, 
                    ADDR adress = ADS1115_ADDR_GND, 
                    CHANNEL channel = ADS1115_CHANNEL_AIN2_GND,
                    RANGE range = ADS1115_RANGE_4P096V, 
                    RATE rate = ADS1115_RATE_860SPS, 
                    COMPARE compare = ADS1115_BOOL_TRUE);

  bool start() override
  {
    return startContinuousRead();
  }
  void stop() override
  {
  ...
  DriverType type() const override { return this->type_; }
