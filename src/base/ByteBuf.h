 ...
 private:
  ...
  T getT(int32_t index, uint8_t max_bit) const {
    ...
    for (uint8_t delta = 0; delta < max_bit; delta++) {
      value += (originData.at(delta + index) & 0xff)
               << (8 * (max_bit - delta - 1));
    }
    return value;
  }
  ...
  T readT(uint8_t max_bit) {
    ...
    T value = 0;
    for (uint8_t delta = 0; delta < max_bit; delta++) {
      value += (originData.at(delta + readerIndex) & 0xff)
               << (8 * (max_bit - delta - 1));
    }
    readerIndex += max_bit;
    return value;
...
class ByteBuf {
 ...
 public:
   ...
   * 转换成16进制字符串
   */
  std::string toHexString(Case type = Case::Lower,
                          const std::string &fill_str = "") const;
   ...
   * 如果readerIndex超出了buffer内数据的长度，则readerIndex = buffer.length() -
   * 1; 如果writerIndex超出了buffer内数据的长度，则writerIndex = buffer.length()
   */
